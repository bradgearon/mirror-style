...
var process = function (id, themeName) {
    ...
    request.get(url)
        .end(function (res) {
            ...
            var template = fs.readFileSync('variables.mustache', fileOpts),
                templateLess = fs.readFileSync('template.less', fileOpts),
                variablesLess = _.template(template, props);
            ...
            fs.writeFileSync('.tmp/variables.less', variablesLess, fileOpts);
            less.render(templateLess, function (e, css) {
                fs.writeFileSync('out/' + props.name + '.css', css, fileOpts);
            });
        });
};
